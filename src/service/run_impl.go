...
func (s *run) getFileNameWithoutExt(fileName string) string {
	return filepath.Base(fileName[:len(fileName)-len(filepath.Ext(fileName))])
}

func (s *run) Run(name string, env string) *model.API {
	// 共通設定
	...
	api.Request = &model.APIRequest{
		...
		Params:  strings.Trim(s.convertPrettyJSON(params), "\n"),
	}
	api.Response = &model.APIResponse{
		Time:       runTime,
		StatusCode: statusCode,
		Body:       strings.Trim(s.convertPrettyJSON(body), "\n"),
	}

	// 結果を表示
	out := s.tRepo.GetMarged(s.configDir+"print.tmpl", api)
	fmt.Println(out)
...
func (s *run) convertPrettyJSON(body []byte) string {
...
func NewRun(
	...
	fRepo repository.File,
	hRepo repository.HTTPClient,
	tRepo repository.TemplateClient) Run {
	return &run{
		configDir: configDir,
		runsDir:   runsDir,
		fRepo:     fRepo,
